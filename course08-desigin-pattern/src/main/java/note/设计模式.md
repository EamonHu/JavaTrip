...
> 迭代器模式又称为游标模式，它提供一种顺序访问集合/容器中对象元素的方法，而又无须暴露集合内部。迭代器模式可以为不同的容器提供一致的遍历行为，而不用关心容器内容元素组成结构，属于行为型模式。

迭代器模式的本质是抽离集合对象迭代行为到迭代器中，提供一致访问接口。从集合对象角度来说，本身涉及的增删改操作，将遍历操作从集合本身转移到迭代器中，也是尽可能分离职责，符合单一职责原则。

### 适用场景

迭代器再生活中也存在很多应用，比如物流中的传送带，不管传送的物品，只关心快递的二维码，在分发时将一个一个检查发送到目的地即可。

将多个对象聚合在一起形成的总体称为集合，集合对象时能够包容一组对象的容器对象。不同集合内部元素聚合结构可能不同，迭代器屏蔽了内部元素获取细节，为外部方位提供一致的元素访问行为，解耦了元素迭代与集合对象间的耦合，并且通过不同的迭代器为同步集合对象提供不同顺序的元素访问行为。适用于以下场景：
...
- 抽象迭代器Iteraotr：抽象迭代器负责定义访问和遍历元素的接口
...
迭代器一般各个集合都为开发者提供好了，但在学习阶段可以手写一个迭代器以加深对迭代器的理解。

例如创建一个员工的集合，集合中的每个元素就是具体员工，然后手写一个迭代器，将每个员工的岗位名称和岗位职责读取出来。
...
可以看出，迭代器主要方法定义是hasNext()和next()，和上面自己写的是一致的。

另外，可以看到remove()方法体在组合模式中见到过，如果实现类不想实现该方法可以抛异常出来。

> 以ArrayList为例，看一下它的继承层次：ArrayList实现了List接口，List接口继承了Collection接口，Collection接口继承了Iterable接口，Iterable接口中包含了一个iterator（）方法，因此到ArrayList中就需要实现该iterator方法，该方法的实现很简单，就是返回一个实现了Iterator接口的迭代器实例。ArrayList中迭代器的实现采用的是内部类的形式，由于内部类可以直接访问外部类的成员变量，所以该迭代器内部类可以很方便地实现Iterator接口中的hasNext（）和next（）方法
...
增加了类的个数，这在一定程度上增加了系统的复杂性。（几乎每种设计模式的通病）



## 命令模式

> 命令模式是对命令的封装，每一个命令是一个操作：请求的一方发出请求要求执行一个操作；接收请求的一方收到请求，并执行操作。名利模式解耦了请求方和接收方，请求方只需请求执行命令，不用关心命名是怎样被接收，怎样被操作以及是否被执行...等等。
>
> 属于行为型模式

软件系统中，行为请求者与行为实现者通常是一种紧耦合的关系，因为这样实现简单明了。但紧耦合关系缺乏扩展性，需要变更时，只能修改源码。命令模式通过请求与实现之间引入一个抽象命令接口，解耦了请求与实现，并且中间件是抽象的，它可以有不同的子类实现，因此具备扩展性。**命令模式的本质是解耦命令请求与处理**。

### 适用场景

当系统的某项操作具备命令语义时，且命令实现不稳定，那么可以通过命令模式解耦请求与实现，利用抽象命令接口使请求方代码结构稳定，封装接收方具体命令实现细节。接收方与抽象命令接口呈现弱耦合，具备良好的扩展性。命令模式适用于以下应用场景：

1. 现实语义中具备“命令“的操作（如命令菜单、shell命令等）
2. 请求调用者和请求接受者需要解耦，使得调用者和接受者不直接交互
3. 需要抽象等待执行的行为，比如撤销操作和恢复等操作
4. 需要支持命令宏
...
- 接受者角色Receiver：该类负责具体实施或执行一个请求

- 命令角色Command：定义需要执行的所有命令行为

- 具体命令角色ConcreteCommand：该类内部维护一个接收者Receiver在其execute方法中调用receiver的相关方法

- 请求者Invoker：接收客户端的命令，并执行命令

通过Command实现了Invoker和Receiver的解耦。Invoker属于业务逻辑区域，是较稳定的结构；Receiver是业务功能功能，是经常发生变动的。如果一个稳定结构依赖不稳定结构，那么导致整个结构不稳定。扩展性增强体现在两个方面：

1. Receiver属于底层细节，可以通过更换不同的Receiver达到不同的细节实现
2. Command接口本身是抽象的，具备扩展属性

### 命令应用

假如我们自己开发出一个播放器，播放器有播放功能、有拖动进度条功能、停止播放功能以及暂停功能，在操作播放器的时候不是直接去调用播放器的方法，而是通过一个控制条去传达指令给播放器内核，具体传达什么指令会被封装成为一个一个的按钮。那么每个按钮就相当于对一条命令的封装。用控制条实现了用户发送指令与播放器内核接收指令的解耦。
...
2. 扩展性良好，可以很容易地加新命令
3. 支持组合命令，支持命令队列
4. 可以在现有命令基础上，增加额外功能（如结合装饰器模式实现日志记录等）

**缺点**

1. 具体的命令类会变多
2. 解耦的同时引入了额外类型结构，增加了程序的理解难度（设计模式的通病）
...
状态模式中的行为是由状态决定的，不同的状态下有不同的行为。其意图是让一个对象在其内部改变的时候，其行为也随之改变。**状态模式核心是状态与行为绑定，不同的状态对应不同的行为。**

### **适用场景**

状态模式在生活中也比较常见。例如：网购的订单状态变化。另外，平时坐电梯，电梯状态的变化。

软开过程中，对于某一项操作，可能存在不同的情况。通常处理多情况问题最直接的方式就是使用if else或者switch case条件遇见进行美剧。但是这种做法对于复杂状态的判断存在弊端：条件判断语句臃肿，且不具备扩展性，维护难度大。

使用状态模式可以将不同状态独立起来用各个不同的类进行表示，消除判断语句的冗余，使得代码更有层次性具有良好扩展力。

状态模式主要解决就是当一个对象状态的条件表达式过于复杂时的情况。通过把状态的判断逻辑转移到不同的状态的一系列类中，可以把复杂的判断逻辑简化。对象行为依赖它的状态，并且会根据它的状态的改变而改变它相关行为。适用于以下场景：
...
1. 环境类角色Context：定义客户端需要的接口，内部维护一个当前状态实例，并负责具体状态的切换
2. 抽象状态角色State：定义该状态下的行为，可以有一个或多个行为
3. 具体状态角色ConcreteState：具体实现该状态对应的行为，并且在需要的情况下进行状态切换

### **状态应用**

在工作学习中通常会使用到博客，对好的文章会进行评论和收藏。如果在登录状态下，可以直接评论和收藏；未登录时，就会提示先登录，再进行登录后的操作。下面用状态模式来实现一下这个逻辑，代码如下：
...
状态模式还经常应用在电商业务开发、游戏系统中等。Spring也提供了一个很好的解决方案，实现的组件名称叫StateMachine(状态机)。状态机帮助开发者简化状态控制的开发过程，让状态机结构更加层次化。有兴趣的可以深入学习

### 优缺点

**状态模式与责任链模式**

> 状态模式和责任链模式都能消除if分支过多的问题。
>
> 在某些情况下，状态模式中的状态可以理解为责任，那么在这种情况下，两种模式都可以使用。
>
> 从定义上来看，状态模式强调的事一个对象内在状态的改变，而责任链模式强调的事外部节点对象间的改变。
>
> 从代码实现上来看，状态模式是知道自己下一个要进入的状态对象；而责任链模式不清楚下一个节点处理对象。

**状态模式与策略模式**

> 状态模式和策略模式的UML类图架构几乎完全一样，但他们的应用场景不一样。
>
> 策略模式多种算法行为择其一都能够满足，彼此之间是独立的，用户可以自行更换策略算法；
>
> 状态模式各个状态之间是存在相关关系的，彼此之间在一定条件下存在自动切换状态效果，且用户无法指定状态，只能设置初始状态。

**优点**

1. 结构清晰：将状态独立为类，消除冗余的if分支判断，代码更加简洁，扩展性更好
2. 状态转换显示化：通常对象状态的切换是通过赋值进行表现，不够直观；状态类能够使得切换时目的更加明确。
3. 状态职责明确且具备扩展性
...
3. 状态模式对开闭原则的支持并不友好，对于可以切换状态的状态模式，增加新的状态类需要修改那些负责状态转换的源代码，否则无法切换到新增状态。

## 备忘录模式

> 备忘录模式又称为快照模式或令牌模式，是指在不破坏封装的前提下，捕获一个对象的内部状态，并在对象之外保存这个状态。这样以后可以将对象恢复到原先保存的状态。
>
> 属于行为性模式。

在软件系统中，备忘录模式可以提供一个“后悔药”机制，通过存储系统各个历史状态的快照，使得可以在任意时刻将系统回滚到某一个历史状态。

### 适用场景

对于程序员来说，几乎每天都是体验备忘录模式，每天使用的Git、SVN以及本地的代码存储记录，它们都可以提供一种代码版本撤回的功能。还有一个比较贴切的场景——游戏的存档，下次打开还可以继续从之前的位置继续游戏。

适用于以下场景：

1. 需要保存历史快照的场景
2. 希望在对象之外保存状态，且除了自己其他类无法访问状态保存具体内容

通用UML类图如下：



从UML类图可以看到备忘录模式主要包含三个角色：

- 发起人角色Originator：负责创建一个备忘录，记录自身需要保存的状态；具备状态回滚功能
- 备忘录角色Memento：用于存储发起人角色的内部状态，且可以防止发起人意外的对象进行访问
- 备忘录管理员角色Caretaker：负责存储，提供管理备忘录，无法对备忘录内容进行操作和访问

### 备忘录应用

网页中的富文本编辑器，通常会有草稿箱、撤销等操作。下面用代码实现这样一个功能：
...
1. 简化发起人实体类Originator职责，隔离状态存储于获取，实现了信息的封装，客户端无需关心状态保存细节
2. 提供状态回滚功能
...
> 中介者模式又称为调节者模式或调停者模式。用一个中介对象封装一系列的对象交互，中介者使各对象不需要显示地相互作用，从而使其耦合松散，而且可以独立地改变它们之间地交互。
>
> 属于行为型模式

中介者模式包装了一系列对象相互作用地方式，使得这些对象不必相互明显作用。从而使它可以松散欧耦合。当某些对象之间地作用发生改变时，不会立即影响其他地一些对象之间的作用。保证这些作用可以彼此独立的变化。其核心思想是，**通过中介者解耦系统各层次对象的直接耦合，层次对象的对外依赖通信统统交由中介者转发**。

### 适用场景

通过中介者k而言将网状结构松耦合为星状结构，这样一个类的变化只影响中介者和类本身。

![](.\pics\中介者1.png)

![](.\pics\中介者2.png)

日常中见到的信息交易平台，也是中介者模式的体现。中介者模式降低多个类之间的通信复杂性，将系统各层次的多对多关系变成一对多的关系，达到降低系统复杂性，提高扩展性的作用。适用于以下场景：

1. 系统中对象之间存在复杂的引用关系，产生的互相依赖关系结构混乱且难以理解
...
从UML类图中，可以看到，中介者模式通常包含4个角色：

- 抽象中介者Mediator：定义统一的接口，用于各同事角色之间的通信
- 具体中介者ConcreteMediator：从具体的同事对象接收消息，向具体同事对象发出命令，协调各同事间的协作
- 抽象同时类Colleague：每一个同事对象均需要依赖中介者角色，与其他同事间通信时，交由中介者进行转发协作
- 具体同时类ConcreteColleague：负责实现自发行为，转发依赖方法交由中介者进行协调

### 中介者应用

通过中介者模式设计群聊场景，用户向聊天室发送消息，聊天室会向所有的用户显示消息。实际上就是用户发信息与聊天室显示的通信过程，不过用户无法直接将信息发给聊天室，而是需要将信息先发到服务器上，然后服务器再将该消息发给聊天室进行显示。示例代码如下：
...
聊天室简单示范了中介者的思想，示例代码中如果采用接口实现可以更好扩展，包括真实的聊天室会对用户信息采用享元模式进行缓存。设计模式重要的是其思想，看重的是其道，而不是术。

### 优缺点

**优点**

1. 减少类间的依赖，将多对多依赖转化成了一对多，降低了类间耦合
2. 类间各司其职，符合迪米特法则

**缺点**

1. 中介者将原本多个类之间的互相依赖变成了中间者和多个同事类的依赖关系。当同事类越多时，中介者就会越臃肿，变得复杂且难以维护。

## 解释器模式

> 解释器模式是指给定一门语言，定义它的文法的一战过后表示，并定义一个解释器，该解释器使用该表示来解释解释语言中的句子。是一种按照规定的语法进行解析的模式。
>
> 属于行为型模式。

类比于编译器可以将源码解释为机器码，让CPU进行识别并执行。解释器模式的作用与编译器模式一样，都是将一些固定的文法进行解释，构建出一个解释句子的解释器。

简单理解，解释器是一个简单语法分析工具，可以识别句子语义，分离终结符号和非终结符号，提取出需要的信息。

### 适用场景

生活中的乐谱，战争年代发明的摩斯密码，都是一种解释器。程序中，如果存在一种特定类型的问题，该类型问题涉及多个不同实例，但是具备固定文法描述，那么可以使用解释器模式对该类型问题进行解释，分离出需要的信息，根据获取的信息做出相应处理。简而言之，对于一些固定文法构建一个句子的解释器。适用以下场景：
...
一般解释器会定义一个解释方法interpret，交由子类进行具体解释；子类实现一般有两大类型：终结符表达式和非终结符表达式。

例如在公式
$$
R=R1+R2
$$
中，R1和R2就是终结符，+是非终结符。终结符和非终结符分别对应的解释器就是终结符表达式。通常一个终结符只有一个表达式，非终结符表达式根据逻辑的复杂程度而增加，原则上每个文法规则都对应一个非终结符表达式。

### 解释器应用

通过解释器模式实现一个数学表达式计算器，包含加减乘除运算。
...
1. 扩展性强：语法规则修改时，只需要修改相应的非终结表达式即可；扩展语法时，只需要添加相应非终结符类即可
2. 易于实现文法：解释器模式的文法应当时比较简单且易于实现的，过于复杂的语法并不适合使用解释器模式。

**缺点**

1. 语法规则较为复杂时，引起类膨胀；解释器模式每一个语法都要产生一个非终结符表达式，当语法规则比较复杂时，会产生大量的解释类，增加系统维护困难。
2. 执行效率较低：解释器模式采用递归调用的方法

## 访问者模式

> 访问者模式是一种将数据结构与数据操作分离的设计模式。是指封装一些作用于某种数据结构中的各元素的操作，它可以在不改变数据结构的前提下定义作用于这些元素的新操作。
>
> 属于行为型模式。

访问者模式是一个较复杂的设计模式，且使用频率不高。其基本思想是：针对系统中拥有固定类型数的对象结构（元素），在其内提供一个accept()方法用来接受访问者对象的访问。不同的访问者对同一元素的访问内容不同，使得相同的元素集合可以产生不同的数据结果。accept()方法可以接收不同的访问者对象，然后在内部将自己（元素）转发到接收者访问对象的visit()方法内。访问者内部对应的visit()方法就会得到回调执行，对元素进行操作。也就是i通过两次动态分发（第一次是对访问者分发accept()方法，第二次是对元素的分发visit()方法），才最终将一个具体的元素传递到要给具体的访问者。如此依赖，就解耦了数据结构与操作，且数据操作不会改变元素状态。

访问者的核心是，解耦数据结构与数据操作，使得对元素的操作具备优秀的扩展性。可以通过不同的数据操作类型（访问者）实现对相同元素集的不同操作。

### 适用场景

访问者模式在生活场景中也是非常多的，例如每年的KPI考核，KPI考核标准是相对稳定的，但是参与KPI考核的员工可能每年都会发省变化，那么员工就是访问者。再例如去食堂吃饭，餐厅的菜单和就餐方式是相对稳定的，但是去餐厅就餐的人员每天发生变化，因此就餐人员就是访问者。

当系统中存在类型数据稳定的一类数据结构时，可以通过访问者模式方便地实现对该类型所有数据结构地不同操作，而不会对数据产生任何副作用（脏数据）。

简而言之，就是对集中不同类型数据进行多种操作，则使用访问者模式。下面总结以下访问者模式地适用场景：

1. 数据结构稳定，作用于数据结构地操作经常发生变化地场景
2. 需要数据结构与数据操作分离地场景
...
抽象访问者Visitor：接口或抽象类，该类冠以对每一个具体元素的访问行为方法，其参数就是具体元素对象。理论上来说，Visitor方法的个数与元素个数是相等的。如果元素个数经常发生变动，会倒置Visitor方法也要进行变动，这种情形不适用访问者模式。

具体访问者ConcreteVistor：实现对具体元素的操作

抽象元素Element：接口或抽象类，定义了一个接收访问者访问的方法Accept()，表示所有元素类型都支持被访问者访问；

抽象元素Element：接口或抽象类，定义了一个接受访问者被访问的方法accept()，表示所有元素类型都支持被访问者访问；

具体元素ConcreteElement：具体元素类型，提供接受受访问者的具体实现。通常的实现都为：Visitor.vist(this)

结构对象ObjectStructure：该类内部维护了元素集合，并提供方法接受访问者对集合所有元素进行操作。

### 访问者应用

年底KPI考核，员工有研发和项目经理；管理层有CEO和CTO。那么CTO关注工程师的代码量、经理的产品数量；CEO关注工程师的KPI和经理的KPI以及新产品数量。

由于CEO和CTO对不同员工的关注点是不同的，进行访问的时候需要进行不同的处理。
...
访问者最大的优点就是增加访问者非常容易，从代码中可以看到，如果要增加一个访问者，只要新实现一个访问者接口的类，从而达到数据对象与数据操作相分离的效果。如果不使用访问者模式，而又不想对不同的元素进行不同的操作，那么必定需要使用IF-ELSE和类型转换，这样使得代码难以升级维护。
...
静态分派在编译期间就可以确定方法的版本，典型的就是Java重载。

**动态分派**

动态分派不是在编译期间确定方法的版本，而是在运行期间才能确定。最典型的例子就是多态：
...
Java是静态多分派、动态单分派的语言。Java低层不支持动态的双分派，但是可以通过使用设计模式，也可以在Java语言里实现伪动态双分派。在访问者模式中使用的使用的就是伪动态双分派。所谓动态双分派就是运行时依据两个实际类型去判断一个方法的运行行为，而访问者模式实现的手段是进行了两次动态双分派来达到这个效果。

### 优缺点

1. 解耦了数据结构与数据操作，使得待操作集合可以独立变化
...
1. 无法增加元素类型；违背了开闭原则
2. 具体元素变更困难：元素属性变更时，访问者均需要相应修改
3. 违背了依赖倒置原则：为了达到“区别对待”，访问者依赖的时具体元素类型，而不是抽象

## 观察者模式

> 观察者模式，又叫发布-订阅模式。定义一种一对多的依赖关系，一个主题对象可被多个观察者对象同时监听，使得每当主题对象状态变化时，所有依赖于它的对象都会得到通知并被自动更新。
>
> 属于行为型模式。

观察者模式的核心时将观察者和被观察者解耦，以类似于消息/广播发送的机制联动两者，使被观察者的变动能通知到感兴趣的观察者们，从而得做出相应的响应。

### 适用场景

朋友圈的动态提醒，博客的消息提醒，邮件通知等都是观察者模式的应用。

在软件系统中，当系统一方行为依赖于另一方行为的变动时，可使用观察者模式耦合联动双方，使得一方的变动可以通知到感兴趣的另一方对象，从而让另一方对象对此做出响应。观察者模式适用于以下几种场景：

1. 当一个抽象模型种包含两个方面内容，其中一个方面依赖于另一个方面
2. 其他一个或多个对象的变化依赖于另一个对象的变化
3. 实现类似广播机制的功能，无需知道具体收听者，只需分发广播，系统中感兴趣的对象会自动接收此广播
4. 多层级嵌套使用，形成一种链式触发机制，使得事件具备跨域（跨越两种观察者类型）通知。

![](.\pics\观察者模式通用类图.png)

- 抽象主题Subject：指被观察的对象Observable。该角色时一个抽象类或接口，定义了增加、删除、通知观察者对象的方法

- 具体主题ConcreteSubjct：具体被观察者，当其内部状态变化时，会通知已注册的观察者
- 抽象观察者Observer：定义了响应通知的更新方法
- 具体观察者ConcreteObserver：在得到状态更新时，会自动做出响应

### 观察者应用

例如在知乎发布提问时，如果指定了人员回答，对方就会接收到邮件吗通知，这就是观察者模式的一种应用场景。可以使用MQ、消息队列等，其实JDK本身也提供了这样的API。
...
2. 分离了表示层的观察者和数据逻辑层被观察者，并且建立了一套触发机制，使得数据的变化可以响应到多个表示层上
3. 实现了一对多的通信机制，支持事件注册机制，支持兴趣分发机制，当被观察者触发事件时，只有感兴趣的观察者可以接收到通知

**缺点**

1. 如果观察者数量过多，事件通知比较耗时
2. 事件呈现线性关系，如果一个观察者阻塞会影响后续的观察者
3. 观察者和被观察者如果存在依赖关系，会造成两者之间的循环调用，导致系统崩溃